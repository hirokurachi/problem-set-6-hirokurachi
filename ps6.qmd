...
alt_types = ["Quantitative", "Nominal", "Ordinal", "Quantitative", "Nominal", "Nominal",
             "Nominal", "Nominal", "Nominal", "Nominal", "Ordinal", "Quantitative", "Ordinal"]
...
df_geo = pd.DataFrame(base_list_geo.tolist(), columns=[
                      "longitude", "latitude"])

# Remove redundant strings from the columns
df_geo["longitude"] = df_geo["longitude"].map(
    lambda x: float(x.replace("POINT(", "")))
df_geo["latitude"] = df_geo["latitude"].map(
...
df_top_alerts_maps_byhour.to_csv(
...
We cannot define the range of numbers for format "HH:MM" with ".range()" method. Also, the column "hour" contains strings, thus we cannot get range from chosen hours based on the column. 

b. 

```{python}
# Create base column for calculating range of hours
df_top_alerts_maps_byhour["hour_dt"] = pd.to_datetime(
    df_top_alerts_maps_byhour["hour"])
df_top_alerts_maps_byhour["hour_dt"] = [x.time()
                                        for x in df_top_alerts_maps_byhour["hour_dt"]]
...
# Plot the alerts by longitude and latitude for "Jam-Heavy Traffic" cases, for 3 timings
chart_alert = alt.Chart(df_top_alerts_maps_byhour_range).mark_point(
    color="firebrick",
    filled=True
).encode(
    longitude="longitude:Q",
...
chart_alert_map_byhour_range = alt.layer(chart_map, chart_alert).properties(
    title="Top 10 Areas of 'Jam - Heavy Traffic' Alerts Number"
)

chart_alert_map_byhour_range.show()
```
