...
subtype_unique_origin = df_waze["subtype"].unique().tolist()

print(f"type: {type_unique}")
print(f"subtype: {subtype_unique_origin}")
...
```{python}
# Remove heads (types) from each subtypes to isolate subtypes from types
# Define function to remove head
def remove_head(df, heads, basecol):
    result = {}
    for head in heads:
        df_subset = df[df[basecol].str.contains(head, na = False)]
        subtype_unique_subset = df_subset["subtype"].unique().tolist()
        subtype_unique_subset = [x.replace(f"{head}_", "") for x in subtype_unique_subset if isinstance(x, str)]
        result[head] = subtype_unique_subset
    return result
# Isolate subtypes from types by the function
dic_subtype_unique_isolated = remove_head(df_waze, type_unique, "type")

print(dic_subtype_unique_isolated)
```
...
def extract_subtype(dic):
    result = []
    for key in dic.keys():
        if key in ["JAM", "ACCIDENT", "ROAD_CLOSED"]:
            add = dic[key]
            result.append(add)
        elif key == "HAZARD":
            add = ["ON_ROAD", "ON_SHOULDER", "WEATHER"]
            result.append(add)
    return result
subtype_unique = extract_subtype(dic_subtype_unique_isolated)

# Isolate sub-subtypes from subtypes
# Parallel to isolating subtypes; remove heads (extracted subtypes) from each subtypes
df_crosswalk["updated_subsubtype"] = [remove_head(df_crosswalk, x, "subtype") for x in df_crosswalk["subtype"]]



df_crosswalk["updated_type"] = [x.replace("_", " ").capitalize() for x in df_crosswalk["type"]]
df_crosswalk


df_waze.groupby(["type", "subtype"]).size().reset_index()

print(df_crosswalk)

df_crosswalk["updated_subtype"] = [x.replace("_", " ").capitalize() for x in subtype_unique]


df_crosswalk = pd.DataFrame({
    "type" : type_unique + [np.nan],
    "subtype" : subtype_unique_origin + [np.nan],
    "updated_type" : [x.replace("_", " ").capitalize() for x in type_unique],
    "updated_subtype" : [x.replace("_", " ").capitalize() for x in subtype_unique],
    "updated_subsubtype" : [x.replace("_", " ").capitalize() for x in subsubtype_unique]
})
