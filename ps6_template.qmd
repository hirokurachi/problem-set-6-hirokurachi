...
base_list_geo = df_waze_updated["geo"].map(lambda x: re.split("\s", x))

# Convert the series of lists into df
df_geo = pd.DataFrame(base_list_geo.tolist(), columns = ["latitude", "longitude"])

# Remove redundant strings from the columns
df_geo["latitude"] = df_geo["latitude"].map(lambda x: float(x.replace("POINT(", "")))
df_geo["longitude"] = df_geo["longitude"].map(lambda x: float(x.replace(")", "")))
...
df_waze_updated[["latitude", "longitude"]] = df_waze_updated[["latitude", "longitude"]].applymap(lambda x: round(x, 2))

# Count each group by latitude and longitude
summary_df_geo = df_waze_updated.groupby(["latitude", "longitude"]).size().reset_index()
summary_df_geo.columns = ["latitude", "longitude", "count"]
summary_df_geo = summary_df_geo.sort_values("count", ascending = False)
...
df_top_alerts_maps = df_waze_updated.groupby(["updated_type", "updated_subtype", "latitude", "longitude"]).size().reset_index()
df_top_alerts_maps.columns = ["updated_type", "updated_subtype", "latitude", "longitude", "count"]

# Group and filter top 10 from the sorted df by type and subtype
df_top_alerts_maps = df_top_alerts_maps.sort_values("count", ascending = False).groupby(["updated_type", "updated_subtype"]).head(10).reset_index(drop = True)

df_top_alerts_maps.to_csv(r"top_alerts_map\top_alerts_map.csv")
```

The level of aggregation is . And the DataFrame has 155 rows.

2. 
```{python}
# Plot the alerts by longitude and latitude for "Jam-Heavy Traffic" cases
chart_alert = alt.Chart(df_top_alerts_maps).mark_point(color = "firebrick").encode(
    alt.X("longitude:Q", scale = alt.Scale(domain = [41.60, 42.00])),
    alt.Y("latitude:Q", scale = alt.Scale(domain = [-87.80, -87.60])),
    alt.Size("count:Q", scale = alt.Scale(domain = [2400, 4500]), legend = alt.Legend(title = "Number of Alerts"))
).properties(
...
#     latitude = "latitude:Q",
#     size = alt.Size("count:Q", scale = alt.Scale(domain = [2400, 4500]), legend = alt.Legend(title = "Number of Alerts"))
# ).properties(
#     title = "Top 10 Areas of 'Jam - Heavy Traffic' Alerts Number",
#     height = 300,
#     width = 300
