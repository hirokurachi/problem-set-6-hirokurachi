...
summary_df_geo = df_waze_updated.groupby(["latitude", "longitude"]).size().reset_index().rename(columns = {0 : "count"}).sort_values("count", ascending = False)
...
df_top_alerts_maps = df_waze_updated.groupby(["updated_type", "updated_subtype", "latitude", "longitude"]).size().reset_index().rename(columns = {0 : "count"})
...
chart_alert = alt.Chart(df_top_alerts_maps).mark_point(color = "firebrick", filled = True).encode(
