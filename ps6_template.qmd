...
df_top_alerts_maps = df_waze_updated.groupby(
    ["updated_type", "updated_subtype", "latitude", "longitude"]
).size().reset_index().rename(
    columns = {0 : "count"}
)
...
df_top_alerts_maps_byhour = df_waze_updated.groupby(
    ["updated_type", "updated_subtype", "latitude", "longitude", "hour"]
).size().reset_index().rename(
    columns = {0 : "count"}
)
...
def plot_chart_alert(time):
    df = df_top_alerts_maps_byhour[df_top_alerts_maps_byhour["hour"] == time]
    chart_alert = alt.Chart(df).mark_point(
        color = "firebrick",
        filled = True
    ).encode(
        longitude = "longitude:Q",
    ...
    chart_alert_map_byhour = chart_map + chart_alert
    return chart_alert_map_byhour
# Apply the function
plot_chart_alert("06:00")|plot_chart_alert("12:00")|plot_chart_alert("23:00")
##NEED TO FIX
```
