...
```{python}
# Print Unique types and subtypes
...
def subtype_isna(type):
    df_waze_subset = df_waze[df_waze["type"] == type]
    subtype_unique_subset = df_waze_subset["subtype"].unique().tolist()
    if np.nan in subtype_unique_subset:
        return True
    else:
        return False
# Apply the function in list comprehension
type_subtype_isna = [type for type in type_unique if subtype_isna(type)]
print(f"types which includes subtype nan: {type_subtype_isna}")

# Can identify sub-subtype?
